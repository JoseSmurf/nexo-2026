...
| replayed request (including after restart) | repeated execution of old signed request | request id replay cache; optional distributed replay guard; persistent replay can be required via `NEXO_REQUIRE_PERSISTENT_REPLAY` | `src/api/auth.rs`, `src/api/replay.rs`, `src/api.rs`, `docs/SECURITY_OPERATIONS.md` | local in-memory replay does not survive restart; production posture depends on requiring a persistent backend |
