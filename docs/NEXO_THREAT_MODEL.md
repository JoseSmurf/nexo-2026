...
| replayed request (including after restart) | repeated execution of old signed request | request id replay cache; optional distributed replay guard; persistent replay can be required via `NEXO_REQUIRE_PERSISTENT_REPLAY`; `request_id` is consumed as a one-time signed nonce before audit persistence | `src/api/auth.rs`, `src/api/replay.rs`, `src/api.rs`, `docs/SECURITY_OPERATIONS.md` | local in-memory replay does not survive restart; production posture depends on requiring a persistent backend; same `request_id` retry after append failure is intentionally rejected |
...
| exposed or abused `/api/chat/send` ingress | operator-noise spam or avoidable request-memory pressure on non-authoritative surface | loopback-only gate, explicit 4 KiB body cap, explicit JSON content-type validation with duplicate header rejection | `src/api.rs`, `docs/SECURITY_OPERATIONS.md` | route is intentionally not signed/replay-protected like `/evaluate`; hostile deployments must keep it private/local-only |
...
| service starts with pre-existing audit artifact incident | startup/ready signal may not reflect existing lock/temp/chain corruption | optional fail-closed startup preflight via `NEXO_REQUIRE_AUDIT_PREFLIGHT=true` validates persisted artifact integrity snapshot before serving traffic | `src/api.rs`, `src/audit_store.rs`, `docs/SECURITY_OPERATIONS.md` | opt-in control; startup snapshot does not prove never-persisted events and is not a distributed lock model |
| multi-process writes to same audit path | lost update or inconsistent continuity due to writer races | fail-closed lock-file guard (`<audit_path>.lock`) and documented single-writer contract per `NEXO_AUDIT_PATH` | `src/audit_store.rs`, `docs/SECURITY_OPERATIONS.md`, `docs/OPERATIONAL_FLOW.md` | lock file is local/process-level only; shared-filesystem/replica deployments still require external coordination |
| operator recovery mistakes during audit incidents | evidence corruption, unverifiable claims, or unsafe resume | preservation-first quarantine checklist, explicit forbidden actions, offline verification role, and persistence-outcome decision table | `docs/SECURITY_OPERATIONS.md`, `docs/OPERATIONAL_FLOW.md` | docs guide operator action but cannot enforce every manual recovery step in code |
| dependency/build-script/proc-macro/CI supply-chain compromise | build-time or CI trust boundary may introduce unreviewed execution paths into release artifacts | `cargo deny check`, lockfile source restrictions, and local regression guard (`scripts/check_supply_chain_surface.sh`) that rejects repo `build.rs` and local unsafe/FFI/process-exec patterns in sensitive paths | `deny.toml`, `.github/workflows/rust.yml`, `scripts/check_supply_chain_surface.sh` | transitive dependencies can still execute build scripts; external CI actions/tool installers remain external trust; duplicate versions are currently warn-level triage; regression checks are guardrails, not formal proof |
