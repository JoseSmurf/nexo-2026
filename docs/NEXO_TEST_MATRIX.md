...
| profile selection | `NEXO_PROFILE` unset defaults to `br_default_v1`; explicit unknown profile names fail closed | `src/profile.rs` | Run individually: `cargo test -q unset_profile_env_defaults_to_br` / `...unknown_explicit_profile_env_panics_fail_closed` / `...known_profile_lookup_returns_expected_profile` | Deployment profile typos may silently select the wrong policy profile. |
...
| record chain semantics | `prev_record_hash` / `record_hash` chaining and single-writer guard semantics stay intact | `src/audit_store.rs`, `src/audit/record.rs` | Run individually: `cargo test -q append_adds_chain_fields` / `...audit_store_append_fails_when_lock_file_already_exists` / `...audit_store_append_with_existing_lock_preserves_existing_file_content` / `...compute_record_hash_is_stable_for_known_input` | Tamper/continuity detection weakens; archive integrity assumptions may break. |
| startup audit preflight (optional) | hostile deployments can fail closed before serving traffic when existing audit artifact has lock/temp/integrity incidents | `src/api.rs`, `src/audit_store.rs` | Run individually: `cargo test -q audit_preflight_helper_is_noop_when_disabled` / `...audit_preflight_helper_fails_when_enabled_and_artifact_is_malformed` / `...preflight_fails_when_lock_file_exists` / `...preflight_fails_when_temp_file_exists` / `...preflight_fails_on_prev_record_hash_mismatch` | Service may report ready/start while pre-existing audit artifact incidents are unresolved. |
...
| `/evaluate` HTTP body/content-type boundary | `/evaluate` enforces a bounded body and strict JSON content-type gate | `src/api.rs` | Run individually: `cargo test -q oversized_body_returns_body_too_large_even_with_invalid_signature` / `...missing_content_type_returns_415` / `...non_json_content_type_returns_415` / `...duplicate_content_type_returns_415` / `...application_json_with_charset_is_accepted` | Oversized or ambiguous payloads may reach deeper security/decision paths unexpectedly. |
| `/api/chat/send` ingress boundary (non-authoritative route) | chat send enforces explicit local ingress limits (JSON content-type, duplicate content-type rejection, 4 KiB cap, loopback-only) without becoming decision/audit authority | `src/api.rs` | Run individually: `cargo test -q api_chat_send_missing_content_type_returns_415` / `...api_chat_send_non_json_content_type_returns_415` / `...api_chat_send_duplicate_content_type_returns_415` / `...api_chat_send_application_json_with_charset_is_accepted` / `...api_chat_send_oversized_body_returns_400` / `...api_chat_send_wrong_content_type_with_oversized_body_returns_415` / `...api_chat_send_handler_rejects_non_loopback_request` / `...api_chat_send_handler_rejects_invalid_channel` / `...api_chat_send_handler_rejects_empty_text` / `...api_chat_send_handler_rejects_long_message` | Non-authoritative chat ingress can drift into avoidable abuse/noise surface if route boundaries are implicit or weakened. |
| replay protection | `request_id` is consumed as a one-time signed nonce; same id is rejected on reuse (including retries after append failure) | `src/api/replay.rs`, `src/api.rs` | Run individually: `cargo test -q request_id_reused_returns_409` / `...same_request_id_after_audit_lock_failure_returns_replay_conflict` | Replay gate weakens or request-id semantics drift into unsafe implicit idempotency. |
| `/evaluate` acceptance/persistence boundary | `200` is returned only after append success; append failures fail closed without decision payload | `src/api.rs`, `src/audit_store.rs` | Run individually: `cargo test -q evaluate_fails_closed_when_audit_lock_preexists_and_does_not_append` / `...evaluate_fails_closed_with_malformed_audit_tail_and_does_not_repair` / `...evaluate_success_persists_audit_record_with_chain_fields` | Accepted-response boundary drifts; operators may lose trust that success maps to persisted audit evidence. |
...
| Residual signal classifier cross-language contract | Julia-generated residual artifact must pass Rust fail-closed contract validation and Zig Blake3 hash parity | `julia/generate_residual_signal_artifact.jl`, `src/residual_signal_contract.rs`, `src/bin/residual_signal_hash.rs`, `tools/zig/src/hash_file.zig`, `.github/workflows/rust.yml` (`residual_signal_classifier_contract` job) | `julia --project=./julia julia/generate_residual_signal_artifact.jl artifacts/residual_signal_classifier/generated_local.json` then `cargo run --quiet --bin residual_signal_hash -- artifacts/residual_signal_classifier/generated_local.json` and `zig run tools/zig/src/hash_file.zig -- artifacts/residual_signal_classifier/generated_local.json <rust_hash>` | Cross-language residual contract drift; Rust and Zig no longer agree on generated artifact byte-level integrity. |
| audit incident runbook contract (docs) | Preservation-first quarantine checklist, forbidden actions, persistence-outcome interpretation, and offline verification role are explicit for operators | `docs/SECURITY_OPERATIONS.md`, `docs/OPERATIONAL_FLOW.md`, `docs/NEXO_THREAT_MODEL.md` | Manual docs review (not an automated runtime test) | Operators may perform unsafe recovery actions even when fail-closed code paths exist. |
| dependency/build-surface guard | local supply-chain regression guard rejects repo-owned `build.rs`, sensitive-path unsafe/FFI/process-exec patterns, and non-crates.io lockfile source drift | `scripts/check_supply_chain_surface.sh`, `Cargo.lock`, `.github/workflows/rust.yml` (security job) | `bash scripts/check_supply_chain_surface.sh` | Repository-local supply-chain boundary can drift silently (unsafe/process exec/build scripts/source-origin changes). |
| dependency policy (`cargo deny`) | advisories/licenses/source policy is enforced; duplicate dependency versions are surfaced for manual review | `deny.toml`, `.github/workflows/rust.yml` (security job) | `cargo deny check` | Vulnerable/unknown-source dependency posture may drift; duplicate version warnings require reviewer/operator triage. |
...
- `bash scripts/check_supply_chain_surface.sh` (CI: security job)
...
- `julia --project=./julia julia/generate_residual_signal_artifact.jl artifacts/residual_signal_classifier/generated_local.json` then `cargo run --quiet --bin residual_signal_hash -- artifacts/residual_signal_classifier/generated_local.json` then `zig run tools/zig/src/hash_file.zig -- artifacts/residual_signal_classifier/generated_local.json <rust_hash>` (CI: residual_signal_classifier_contract job)
...
- No claim that Redis replay behavior under audit-append failure is covered by this patch's local test surface.
