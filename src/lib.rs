...
pub fn audit_hash_with_algo(trace: &[Decision], algo: AuditHashAlgo) -> String {
    match algo {
        ...
        AuditHashAlgo::Shake256_256 | AuditHashAlgo::Shake256_384 | AuditHashAlgo::Shake256_512 => {
            ...
            for d in trace {
                match *d {
                    Decision::Approved => hash_field_shake(&mut h, b"D:A", &[]),
                    Decision::FlaggedForReview {
                        rule_id,
                        reason,
                        severity,
...
mod tests {
    ...
    #[test]
    fn jp_approves_below_night_limit() {
        let cfg = cfg_jp();
        let (night, st) = ts_for_local_time(cfg, cfg.night_start, 0);
        let tx =
            TransactionIntent::new("user_jp_300k", 300_000, false, true, night, st, 1_000, true)
                .unwrap();
    ...
    #[test]
    fn jp_blocks_above_night_limit() {
        let cfg = cfg_jp();
        let (night, st) = ts_for_local_time(cfg, cfg.night_start, 0);
        let tx =
            TransactionIntent::new("user_jp_500k", 500_000, false, true, night, st, 1_000, true)
                .unwrap();
    ...
    #[test]
    fn jp_approves_exactly_at_night_limit() {
        let cfg = cfg_jp();
        let (night, st) = ts_for_local_time(cfg, cfg.night_start, 0);
        let tx = TransactionIntent::new(
            "user_jp_exact",
    ...
    #[test]
    fn jp_blocks_one_cent_above_night_limit() {
        let cfg = cfg_jp();
        let (night, st) = ts_for_local_time(cfg, cfg.night_start, 0);
        let tx = TransactionIntent::new(
            "user_jp_one_cent",
    ...
    #[test]
    fn gb_approves_below_night_limit() {
        let cfg = cfg_gb();
        let (night, st) = ts_for_local_time(cfg, cfg.night_start, 0);
        let tx =
            TransactionIntent::new("user_gb_200k", 200_000, false, true, night, st, 1_000, true)
                .unwrap();
    ...
    #[test]
    fn gb_blocks_above_night_limit() {
        let cfg = cfg_gb();
        let (night, st) = ts_for_local_time(cfg, cfg.night_start, 0);
        let tx =
            TransactionIntent::new("user_gb_400k", 400_000, false, true, night, st, 1_000, true)
                .unwrap();
    ...
    #[test]
    fn gb_approves_exactly_at_night_limit() {
        let cfg = cfg_gb();
        let (night, st) = ts_for_local_time(cfg, cfg.night_start, 0);
        let tx = TransactionIntent::new(
            "user_gb_exact",
    ...
    #[test]
    fn gb_blocks_one_cent_above_night_limit() {
        let cfg = cfg_gb();
        let (night, st) = ts_for_local_time(cfg, cfg.night_start, 0);
        let tx = TransactionIntent::new(
            "user_gb_one_cent",
    ...
    #[test]
    fn kr_approves_below_night_limit() {
        let cfg = cfg_kr();
        let (night, st) = ts_for_local_time(cfg, cfg.night_start, 0);
        let tx =
            TransactionIntent::new("user_kr_300k", 300_000, false, true, night, st, 1_000, true)
                .unwrap();
    ...
    #[test]
    fn kr_blocks_above_night_limit() {
        let cfg = cfg_kr();
        let (night, st) = ts_for_local_time(cfg, cfg.night_start, 0);
        let tx =
            TransactionIntent::new("user_kr_450k", 450_000, false, true, night, st, 1_000, true)
                .unwrap();
    ...
    #[test]
    fn kr_approves_exactly_at_night_limit() {
        let cfg = cfg_kr();
        let (night, st) = ts_for_local_time(cfg, cfg.night_start, 0);
        let tx = TransactionIntent::new(
            "user_kr_exact",
    ...
    #[test]
    fn kr_blocks_one_cent_above_night_limit() {
        let cfg = cfg_kr();
        let (night, st) = ts_for_local_time(cfg, cfg.night_start, 0);
        let tx = TransactionIntent::new(
            "user_kr_one_cent",
    ...
    #[test]
    fn shake_hash_is_deterministic() {
        let tx = base_tx();
        let (_d, trace, _hash) = evaluate(&tx);
        let hash1 = audit_hash_with_algo(&trace, AuditHashAlgo::Shake256_512);
        let hash2 = audit_hash_with_algo(&trace, AuditHashAlgo::Shake256_512);
        assert_eq!(hash1, hash2);
        assert_eq!(hash1.len(), 128);
    }

    #[test]
    fn hybrid_hash_length_matches_expected() {
        let tx = base_tx();
        let (_d, trace, _hash) = evaluate(&tx);
        let hybrid = audit_hash_with_algo(&trace, AuditHashAlgo::HybridShake512Blake3_256);
        assert_eq!(hybrid.len(), 192);
    }

    #[test]
    fn shake_sizes_match_requested_bits() {
        let tx = base_tx();
        let (_d, trace, _hash) = evaluate(&tx);
        let h256 = audit_hash_with_algo(&trace, AuditHashAlgo::Shake256_256);
        let h384 = audit_hash_with_algo(&trace, AuditHashAlgo::Shake256_384);
        let h512 = audit_hash_with_algo(&trace, AuditHashAlgo::Shake256_512);
